...
const OpportunitiesTable: React.FC<OpportunitiesTableProps> = ({ opportunities }) => {
  ...
  const sortedOpportunities = [...opportunities].sort((a, b) => {
    let cmp = 0;
    if (sortBy === 'time') {
      cmp = new Date(a.opportunity_timestamp).getTime() - new Date(b.opportunity_timestamp).getTime();
  ...
  const totalProfit = opportunities.reduce((sum, o) => sum + parseFloat(o.net_profit || '0'), 0);

  return (
    <div className="bg-gray-900 border border-gray-800">
      <div className="flex items-center justify-between px-3 py-2 border-b border-gray-800">
        <div className="flex items-center gap-2">
          <h3 className="text-xs font-medium text-white">Recent Opportunities</h3>
          <span className="text-[10px] text-gray-500">{opportunities.length} found</span>
        </div>
      ...
      {opportunities.length === 0 ? (
        <div className="p-8 text-center">
          ...
          <p className="text-gray-600 text-[10px] mt-1">Keep the bot running to find arbitrage opportunities</p>
        </div>
      ) : (
                ...
                return (
                  <tr key={opp.id} className="hover:bg-gray-800/30">
                    ...
                    <td className="px-3 py-2 text-right">
                      <span className={`font-mono ${isHighSpread ? 'text-emerald-400' : 'text-gray-300'}`}>
                        {spread.toFixed(4)}%
                      </span>
                    </td>
                    <td className="px-3 py-2 text-right">
                      <span className={`font-mono font-medium ${isHighProfit ? 'text-emerald-400' : 'text-emerald-400/70'}`}>
                        ${profit.toFixed(2)}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-right">
